...
search_dict = {breadth_first_search : 1,
            breadth_first_tree_search : 2,
            depth_first_graph_search : 3 ,
            depth_limited_search : 4,
            uniform_cost_search : 5,
            recursive_best_first_search : 6,
            greedy_best_first_graph_search : 7,
            'h_1' : 8,
            'h_ignore_preconditions' : 9,
            'h_pg_levelsum': 10,
            }

problem_dict = {air_cargo_p1 : 1,
                air_cargo_p2 : 2,
                air_cargo_p3 : 3
                }
...
def datatable(ip, search_id, problem_id, optlen, eltime, df):
    """print("Expansions : ", ip.succs)
    print("Search Function : ", search_id)
    print("Problem : ", problem_id)
    print("Optimal Length : ", optlen)
    print("Elapsed time : ", eltime)"""
    df1 = pd.DataFrame(columns = ["Problem ID","Search ID","Elapsed Time","Optimal Length","Expansions"])
    df1.loc[0] = [problem_id, search_id, eltime, optlen, ip.succs]
    df = df.append(df1, ignore_index=True)
    return df


def run_search(problem, search_function, hstring, df ,parameter=None):

    search_id = 0
    problem_id = problem_dict.get(problem)

    start = timer()
    ip = PrintableProblem(problem())
    if parameter is not None:
        node = search_function(ip, parameter)
        if search_function == astar_search:
            search_id = search_dict.get(hstring)
        else:
            search_id = search_dict.get(search_function)
    else:
    ...
    print()
    df = datatable(ip, search_id, problem_id, len(node.solution()), end - start, df)
    return df
...
def main(p_choices, s_choices):
    df = pd.DataFrame(columns = ["Problem ID","Search ID","Elapsed Time","Optimal Length","Expansions"])
    ...
    for pname, p in problems:
        for sname, s, h in searches:
            ...
            df = run_search(p, s, hstring[6:], df, _h)
