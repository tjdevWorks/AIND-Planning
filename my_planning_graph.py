...
class PlanningGraph():
        ...
        for action in self.all_actions:
            ...
            if action_node.prenodes.issubset(self.s_levels[level]):
                ...
                for snode in self.s_levels[level]:
                    action_node.parents.add(snode)
                    snode.children.add(action_node)
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level-1]:
            literals = action_node.effnodes
            for lnode in literals:
                self.s_levels[level].add(lnode)
                lnode.parents.add(action_node)
                action_node.children.add(lnode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for effa in node_a1.action.effect_add:
            for effr in node_a2.action.effect_rem:
                if effa == effr:
                    return True

        for effa in node_a2.action.effect_add:
            for effr in node_a1.action.effect_rem:
                if effa == effr:
                    return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        for effa in node_a1.action.effect_add:
            for posn in node_a2.action.precond_neg:
                if effa==posn:
                    return True

        for effr in node_a1.action.effect_rem:
            for posp in node_a2.action.precond_pos:
                if effr==posp:
                    return True

        for effa in node_a2.action.effect_add:
            for posn in node_a1.action.precond_neg:
                if effa==posn:
                    return True

        for effr in node_a2.action.effect_rem:
            for posp in node_a1.action.precond_pos:
                if effr==posp:
                    return True
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            for i in range(len(self.s_levels)):
                for n in self.s_levels[i]:
                    if n.is_pos and g==n.symbol:
                        flag=True
                        level_sum+=i
                        break
                if flag:
